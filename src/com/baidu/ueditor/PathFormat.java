...
public class PathFormat {
	 ...
	 * 
	 * @param input
	 * @param dirAndId	courtImg/id,或者是userImg/id，id为实际主键
	 * @param filename
	 * @return
	 */
	public static String parse(String input, String dirAndId, String filename) {
		...
		while (matcher.find()) {
			...
			if (matchStr.indexOf("filename") != -1) {
				...
				matcher.appendReplacement(sb, PathFormat.getStringUsingId(matchStr, dirAndId));
			}
	...
	private static String getStringUsingId(String pattern, String id) {

		pattern = pattern.toLowerCase();
System.out.println("pattern = "+pattern);
System.out.println("index = "+pattern.indexOf(PathFormat.ID));
		// time 处理
		if (pattern.indexOf(PathFormat.ID) != -1) {
