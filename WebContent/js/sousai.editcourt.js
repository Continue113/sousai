...
uploader = WebUploader.create({
  ...
  // 文件接收服务端。
  server: 'http://localhost:8080/sousai/ueditor/jsp/controller.jsp?action=uploadimage',
...
uploader.on( 'fileQueued', function( file ) {
  ...
  var $li = $('<li id="' + file.id + '"><p class="title">' + file.name + '</p><p class="imgWrap"><img></p>'+
		  '<div class="file-panel"><span>删除<span></div></li>'
          	),
      $img = $li.find('img');
  
  $list.append( $li );
  $li.on('click','.file-panel',function(evt){
	 console.log(file);
	 alert("sss");
  });
...
uploader.on('error',function(errType){
	console.log(errType);
	if(errType){
		sousaiRemindDialog("请上传小于 200KB 的 jpg、jpeg、png、gif 格式的图片。图片数量最多为3张.")
	}
});
  });
